...
// prettier-ignore
function getDraft() { const history = document.getElementsByClassName("pick-history")[0]; if (!history) { const s = window.state; const seen = Object.fromEntries(s.map(o => [o.name, true])); const recent = Array.from(document.getElementsByClassName("pick__message-content")).map(e => e.getElementsByClassName("playerinfo__playername")[0]).map(e => e.innerText).filter(name => !seen[name]).map((name, i) => ({ name, rank: s.length + 1 + i })); return s.concat(recent); } return Array.from(history.getElementsByClassName("fixedDataTableCellGroupLayout_cellGroup")).map(row => ({ name_e: row.getElementsByClassName("playerinfo__playername")[0], rank_e: Array.from(row.children).reverse()[0] })).filter(_ref4 => { let { name_e, rank_e } = _ref4; return name_e && rank_e; }).map(_ref5 => { let { name_e, rank_e } = _ref5; return { name: name_e.innerText, rank: parseInt(rank_e.innerText) }; }); }
